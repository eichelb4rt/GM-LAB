...
\begin{algorithm}
	\tcp{bla bla}
	% \uIf{$v \in V$ with $\text{deg}_G(v) = 1$ exists} {
	% 	$v \gets$ an arbitrary node $v$ with $\text{deg}_G(v) = 1$\;
	% 	\If{$\abs{V}$ is odd} {
	% 		$v \gets$ an arbitrary neighbour of $v$\;
	% 	}
	% }
	% \uElseIf{$v \in V$ with $\text{deg}_G(v) = 2$ exists}{
	% 	$v \gets$ an arbitrary node $v$ with $\text{deg}_G(v) = 2$\;
	% }
	% \Else{
	% 	\Return{$\emptyset$}
	% }
	% \tcp{Pick every second node}
	% $W \gets \emptyset$\;
	% \While{$v$ is not marked} {
	% 	$W \gets W \cup \set{v}$\;
	% 	mark $v$\;
	% 	\If{all neighbours of $v$ are marked} {
	% 		\Return{W}
	% 	}
	% 	$w \gets$ an arbitrary neighbour of $v$ that is not marked\;
	% 	mark all neighbours of $v$\;
	% 	$v \gets$ the neighbour of $w$ that is not $v$\;
	% }
	% \Return{$W$}
