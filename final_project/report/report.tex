...
The hill climbing algorithm just applies every possible change that improves the objective function.
It is illustrated in \autoref{func:methods:climb_hill}.
...
\begin{function}
	...
	\While{$S_{max}$ increases}{
		...
		\ForEach{$c \in C$}{
			...
			\eIf{$Score(G, D) > S_{max}$}{
				$S_{max} \gets Score(G', D)$\;
			}{
...
This is also a key point for the efficiency of generating all possible changes, which is illustrated in \autoref{func:methods:all_changes}.
